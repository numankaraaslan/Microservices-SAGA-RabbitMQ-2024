...
public class RouteConfig
{
	...
	public RouteLocator loadBalancedRoutes(RouteLocatorBuilder builder)
	{
		...
		routes.route(activityfaileroute -> activityfaileroute.path("/activity-failover").uri("lb://ab-activityservice-failover"));
		routes.route(activityroute -> activityroute.path("/act/**").filters(fn -> fn.circuitBreaker(cns -> cns.setFallbackUri("forward:/activity-failover"))).uri("lb://ab-activityservice"));
		routes.route(paymentfaileroute -> paymentfaileroute.path("/payment-failover").uri("lb://ab-paymentservice-failover"));
		routes.route(paymentroute -> paymentroute.path("/pay/**").filters(fn -> fn.circuitBreaker(cns -> cns.setFallbackUri("forward:/payment-failover"))).uri("lb://ab-paymentservice"));
